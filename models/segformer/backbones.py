...
from models.segformer.timm import trunc_normal_, drop_path
from models.segformer.GFA import GFA
...
class MixTransformer(nn.Module):
    def __init__(
        ...
        self.gfa = GFA(p=uncertainty_probability)
        self.init_weights()
    ...
    def forward(self, x):
        """
           b0 output: [2 32 128 128], [2 64, 64, 64], [2, 160, 32, 32], [2, 256, 16, 16]
        """
        outputs = []
...
def _mit_bx(in_c, embed_dims: Tuple4i, depths: Tuple4i, uncertainty_probability) -> MixTransformer:
...
def mit_b0(uncertainty_probability, in_c):
    return _mit_bx(embed_dims=(32, 64, 160, 256), depths=(2, 2, 2, 2), in_c=in_c,
                   uncertainty_probability=uncertainty_probability)


def mit_b1(uncertainty_probability, in_c):
    return _mit_bx(embed_dims=(64, 128, 320, 512), depths=(2, 2, 2, 2), in_c=in_c,
                   uncertainty_probability=uncertainty_probability)


def mit_b2(uncertainty_probability, in_c):
    return _mit_bx(embed_dims=(64, 128, 320, 512), depths=(3, 4, 6, 3), in_c=in_c,
                   uncertainty_probability=uncertainty_probability)


def mit_b3(uncertainty_probability, in_c):
    return _mit_bx(embed_dims=(64, 128, 320, 512), depths=(3, 4, 18, 3), in_c=in_c,
                   uncertainty_probability=uncertainty_probability)


def mit_b4(uncertainty_probability, in_c):
    return _mit_bx(embed_dims=(64, 128, 320, 512), depths=(3, 8, 27, 3), in_c=in_c,
                   uncertainty_probability=uncertainty_probability)


def mit_b5(uncertainty_probability, in_c):
    return _mit_bx(embed_dims=(64, 128, 320, 512), depths=(3, 6, 40, 3), in_c=in_c,
                   uncertainty_probability=uncertainty_probability)